...
describe('A FileResourceStore', (): void => {
  ...
  it('can write and read a container.', async(): Promise<void> => {
    ...
    expect(result.metadata.get(CONTENT_TYPE)?.value).toEqual(CONTENT_TYPE_QUADS);
  ...
  it('can set data.', async(): Promise<void> => {
    ...
    expect(result.metadata.get(CONTENT_TYPE)?.value).toEqual('text/plain; charset=utf-8');
  ...
  it('generates a new URI when adding without a slug.', async(): Promise<void> => {
    ...
    expect(filePath.startsWith(root)).toBeTruthy();
    ...
    expect(result.metadata.get(BYTE_SIZE)?.value).toEqual(`${stats.size}`);
    await expect(arrayifyStream(result.data)).resolves.toEqual([ rawData ]);
    expect(fsPromises.lstat as jest.Mock).toBeCalledWith(joinPath(root, name));
    expect(fs.createReadStream as jest.Mock).toBeCalledWith(joinPath(root, name));
    expect(fs.createReadStream as jest.Mock).toBeCalledWith(joinPath(root, `${name}.metadata`));
  });
});
